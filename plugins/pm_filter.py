...
from info import ADMINS, ADMIN, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, FILE_CHANNEL, FILE_FORWARD, SUPPORT_CHAT_LINK
...
#                   await sp.delete()                                                                                   
        ...
        if settings is not None:
            ...
            btn = [[
                InlineKeyboardButton(f'{random.choice(RUN_STRINGS)}𝐉𝐨𝐢𝐧 𝐆𝐫𝐨𝐮𝐩{random.choice(RUN_STRINGS)}', url=f'http://t.me/nasrani_update')
            ], [
                InlineKeyboardButton(f"{random.choice(RUN_STRINGS)}Oᴘᴇɴ Hᴇʀᴇ ↓{random.choice(RUN_STRINGS)}", callback_data=f"opnsetgrp#{grp_id}"),
                InlineKeyboardButton(f"{random.choice(RUN_STRINGS)}Oᴘᴇɴ Iɴ PM ⇲{random.choice(RUN_STRINGS)}", callback_data=f"opnsetpm#{grp_id}")
            ]]
...
#        )
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⇍ ʙᴀᴄᴋ ⇏', callback_data='helps')
        ]]
